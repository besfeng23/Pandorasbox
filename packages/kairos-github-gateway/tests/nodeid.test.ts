import { extractNodeIds, resolveNodeIdsByPriority } from '../src/utils/nodeid';
...
describe('nodeId extraction', () => {
  test('extractNodeIds enforces PB-* regex and is deterministic', () => {
    ...
    expect(extractNodeIds(text)).toEqual(['PB-CORE-CHAT-001', 'PB-OPS-EXPORT-002']);
  });

  test('priority order: title > branch > commit messages > body', () => {
    const res = resolveNodeIdsByPriority({

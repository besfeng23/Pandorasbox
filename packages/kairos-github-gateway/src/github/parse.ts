import { resolveNodeIdsByPriority } from '../utils/nodeid';
import type { KairosEvidenceEvent } from '../kairos/client';

export type GithubWebhookParseResult = {
  repoFullName: string | null;
  events: KairosEvidenceEvent[];
  logs: string[];
};

const WEIGHTS: Record<KairosEvidenceEvent['event_type'], number> = {
...
type WorkflowMapping = { event_type: KairosEvidenceEvent['event_type']; confidence: number } | null;

function workflowRunMapping(workflowName: string | null | undefined): WorkflowMapping {
  ...
  if (/\be2e\b/.test(n) || /\bend[- ]to[- ]end\b/.test(n)) return { event_type: 'github.workflow.e2e_pass', confidence: WEIGHTS['github.workflow.e2e_pass'] };
  if (/\bintegration\b/.test(n)) return { event_type: 'github.workflow.integration_pass', confidence: WEIGHTS['github.workflow.integration_pass'] };
  if (/\bunit\b/.test(n)) return { event_type: 'github.workflow.unit_pass', confidence: WEIGHTS['github.workflow.unit_pass'] };
  if (/\blint\b/.test(n)) return { event_type: 'github.workflow.lint_pass', confidence: WEIGHTS['github.workflow.lint_pass'] };
  return null;
}

function iso(x: any): string {
  const s = typeof x === 'string' ? x : null;
  if (s && !Number.isNaN(Date.parse(s))) return new Date(s).toISOString();
  return new Date().toISOString();
}

function repoFullNameFromPayload(payload: any): string | null {
  const rn = payload?.repository?.full_name;
  if (typeof rn === 'string' && rn.trim()) return rn.trim();
  return null;
}

function dedupeKey(args: { event_type: string; repo: string; pr_number?: number | null; run_id?: number | null; sha?: string | null; node_id: string }): string {
...
export function parseGithubWebhook(args: { eventName: string; payload: any }): GithubWebhookParseResult {
  const logs: string[] = [];
  const repo = repoFullNameFromPayload(args.payload);

  const out: KairosEvidenceEvent[] = [];

  if (!repo) logs.push('missing repository.full_name');
  ...
  if (args.eventName === 'pull_request') {
    ...
    let eventType: KairosEvidenceEvent['event_type'] | null = null;
    if (action === 'opened' || action === 'reopened') eventType = 'github.pr.opened';
    if (action === 'closed' && pr?.merged === true) eventType = 'github.pr.merged';
    if (!eventType) return { repoFullName: repo, events: [], logs };

    const nodeIdResolution = resolveNodeIdsByPriority({
      title: title ?? '',
      branch: branch ?? '',
  ...
  if (args.eventName === 'workflow_run') {
    ...
    if (action !== 'completed' || conclusion !== 'success') return { repoFullName: repo, events: [], logs };
  ...
  if (args.eventName === 'release') {
    const action = String(args.payload?.action ?? '');
    if (action !== 'published') return { repoFullName: repo, events: [], logs };
    ...
    for (const nodeId of nodeIdResolution.nodeIds) {
      const eventType: KairosEvidenceEvent['event_type'] = 'github.release.published';
      out.push({

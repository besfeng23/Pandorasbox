    ...
    <ScrollArea className="flex-1">
        ...
        {isLoading ? (
            <div className="flex justify-center items-center h-full p-8">
                <Loader2 className="animate-spin neon-text-cyan" />
            </div>
        ) : threads.length === 0 ? (
            ...
            <button
                ...
                className={cn(
                'w-full text-left p-2 rounded-md transition-all flex flex-col gap-1',
                activeThreadId === thread.id 
                  ? 'glass-panel border-glow-purple border' 
            ...
            >
                <div className="flex items-start gap-2">
                    <MessageSquare className={cn(
                      ...
                      activeThreadId === thread.id ? "text-neon-purple" : "text-white/40"
                    )} />
                    <p className={cn(
                      "font-semibold text-sm truncate flex-1 pr-4",
                      activeThreadId === thread.id ? "text-white" : "text-white/70"
                    )}>{thread.title}</p>
                </div>

# Add Pandoras Box MCP Server to Cursor Configuration
# This adds pandoras-box in addition to firebase
...
Write-Host "`n=== Adding Pandoras Box MCP Server to Cursor ===" -ForegroundColor Cyan
...
# Create Pandoras Box config object
$pandorasConfig = New-Object PSObject
$pandorasConfig | Add-Member -MemberType NoteProperty -Name "command" -Value "npx"
$pandorasConfig | Add-Member -MemberType NoteProperty -Name "args" -Value @("-y", "tsx", $mcpServerPath)
$pandorasConfig | Add-Member -MemberType NoteProperty -Name "cwd" -Value $projectPath

# Add Pandoras Box to mcpServers (overwrite if exists)
$config.mcpServers | Add-Member -MemberType NoteProperty -Name "pandoras-box" -Value $pandorasConfig -Force

# Write back to file
$config | ConvertTo-Json -Depth 10 | Set-Content $cursorMcpPath -Encoding UTF8

Write-Host "`n✅ Pandoras Box MCP server configuration added!" -ForegroundColor Green
